...
export function assertAgaWorkspaceArtifact(profile, suppliedPath = "dist") {
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  ...
  const result = assertAgaWorkspaceArtifact(profile, artifact);
  console.log(`aga-workspace-artifact-scan: ok (${result.profile}, ${result.files} files, ${result.inputs} inputs)`);
}
