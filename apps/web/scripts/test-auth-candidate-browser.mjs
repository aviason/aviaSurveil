...
try {
  ...
  if (configuration.scopes_supported?.includes("offline_access")) throw new Error("discovery advertises disabled offline_access scope");
  if (configuration.grant_types_supported?.includes("refresh_token")) throw new Error("discovery advertises disabled refresh-token grant");
  ...
  authorize.search = new URLSearchParams({ client_id: "as360-local-candidate-web", response_type: "code", response_mode: "form_post", scope: "openid profile email", redirect_uri: callbackURL, state: "browser-state", nonce: "browser-nonce", code_challenge: challenge, code_challenge_method: "S256" }).toString();
  stage("authorize");
  await page.goto(authorize.toString(), { waitUntil: "domcontentloaded" });
  await page.getByRole("heading", { name: "Sign in" }).waitFor();
  await assertAccessibleForm(page, "Sign in", ["Username or email", "Password"]);
  ...
  await assertAccessibleForm(page, "Verify MFA", ["One-time code", "Use recovery code"]);
  ...
  if (!tokenPayload.access_token || !tokenPayload.id_token || tokenPayload.refresh_token) throw new Error("token response violated the no-refresh-token contract");
  stage("recovery");
  ...
  await assertAccessibleForm(page, "Set a new password", ["New password"]);
