...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`mock governed checklist synthetic lifecycle is accessible and overflow-free at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    const issues: string[] = [];
    ...
    await expect(review.getByRole("button", { name: "Technically approve" })).toBeEnabled();
    await expect(review.getByRole("button", { name: "Publish checklist version" })).toHaveCount(0);
    ...
    await expect(review.getByRole("button", { name: "Publish checklist version" })).toBeEnabled();
