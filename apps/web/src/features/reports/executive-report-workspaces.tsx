...
import type { AuditEventView, DocumentMetadataView, ReportVersionView } from "../../backend/backend";
...
function ExecutiveReportReviewPage({ kind }: { kind: "preliminary" | "final" }) {
  ...
  const [reason, setReason] = useState("");
  const [busy, setBusy] = useState(false);
  const report = selectedId ? reports.find((candidate) => candidate.reportVersionId === selectedId) ?? null : null;
  ...
  async function decide() {
    ...
    try {
      ...
      setReports((current) => current.map((candidate) => candidate.reportVersionId === updated.reportVersionId ? updated : candidate));
      await loadEvents(updated.reportVersionId);
      setReason("");
    } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(false); }
  }
  ...
  return <WorkspaceShell roleLabel="Executive Director" routeLabel={pageName}>
    <div className={`executive-workspace-page ${isPreliminary ? "executive-preliminary-report-page" : "executive-final-report-page"}`} data-testid={pageTestId}>
      ...
      <section aria-label={`${pageName} status summary`} className="executive-report-summary"><article><span>Total</span><b>{reports.length}</b></article><article><span>Pending</span><b>{reports.filter((candidate) => candidate.status === "EXECUTIVE_DIRECTOR_REVIEW").length}</b></article><article><span>Issued / Locked</span><b>{reports.filter((candidate) => candidate.status === "LOCKED").length}</b></article><article><span>Returned</span><b>{reports.filter((candidate) => candidate.status === "RETURNED").length}</b></article></section>
      <section aria-label={`${pageName} report queue`} className="executive-panel executive-report-queue"><header><div><span>Server-owned report versions</span><h2>Select an exact version</h2></div></header>{reports.length ? <div className="responsive-table-shell"><table><thead><tr><th>Report version</th><th>Audit</th><th>Status</th><th>Revision</th><th>Action</th></tr></thead><tbody>{reports.map((candidate) => <tr key={candidate.reportVersionId}><td><b>{candidate.reportVersionId}</b><small>{candidate.reportId}</small></td><td>{candidate.auditId}</td><td>{candidate.status}</td><td>{candidate.revision}</td><td><button disabled={selectedId === candidate.reportVersionId} onClick={() => setSelectedId(candidate.reportVersionId)} type="button">{selectedId === candidate.reportVersionId ? "Selected" : `Open ${candidate.reportVersionId}`}</button></td></tr>)}</tbody></table></div> : <div className="executive-empty"><b>No {isPreliminary ? "Preliminary" : "Final"} Report versions are available.</b><span>The server-owned queue will populate after the report lifecycle creates a version visible to this role.</span></div>}</section>
      {eligibleReport ? <section aria-label={selectedName} className="executive-report-review-layout" data-report-revision={eligibleReport.revision} data-report-version-id={eligibleReport.reportVersionId}><div className="executive-report-review-main"><section className="executive-plan-detail"><header><div><span>{selectedName}</span><h2>{reportLabel(eligibleReport)}</h2><p>{eligibleReport.auditId} · revision {eligibleReport.revision}</p></div><b data-testid="report-status">{eligibleReport.status}</b></header><dl className="executive-definition-grid"><div><dt>Report ID</dt><dd>{eligibleReport.reportId}</dd></div><div><dt>Version ID</dt><dd>{eligibleReport.reportVersionId}</dd></div><div><dt>Version</dt><dd>{eligibleReport.version}</dd></div><div><dt>Revision</dt><dd>{eligibleReport.revision}</dd></div><div><dt>Content hash</dt><dd>{eligibleReport.contentHash}</dd></div><div><dt>Finance report stage</dt><dd>Not applicable</dd></div></dl>{!eligibleReport.findingIds.length ? <p className="executive-report-boundary">No Findings linked — relationship unavailable for {eligibleReport.reportVersionId}</p> : <p>{eligibleReport.findingIds.length} linked Finding{eligibleReport.findingIds.length === 1 ? "" : "s"}</p>}<section><h3>Decision History</h3><DecisionHistory events={events[eligibleReport.reportVersionId] ?? []} /></section>{!isPreliminary ? <Link to={`/executive-director/reports/${encodeURIComponent(eligibleReport.reportVersionId)}`} aria-label={`Preview ${eligibleReport.reportVersionId}`}>Preview Full Report</Link> : null}</section></div><aside><section className="executive-report-decision"><span>Executive Director decision</span><h2>{isPreliminary ? "Preliminary Report authority" : "Final Report authority"}</h2>{eligibleReport.status === "EXECUTIVE_DIRECTOR_REVIEW" ? <><label>Executive Director report decision reason<textarea onChange={(event) => setReason(event.target.value)} value={reason} /></label><button disabled={busy} onClick={() => void decide()} type="button">Issue and lock {eligibleReport.reportVersionId}</button><button aria-label={`Return ${eligibleReport.reportVersionId} unavailable`} disabled title={`Report version ${eligibleReport.reportVersionId} is at Executive Director Review; the typed Plan 1 command permits issue and lock only. Return authority is not declared for Executive Director.`} type="button">Return unavailable</button></> : <button aria-label={`Executive decision unavailable for ${eligibleReport.reportVersionId}`} disabled title={`Report version ${eligibleReport.reportVersionId} is ${eligibleReport.status}; no further Executive Director decision is available.`} type="button">Decision recorded</button>}<p>Report approval never closes a Finding and never applies enforcement.</p></section></aside></section> : <section className="executive-empty"><b>{report ? "No report version is eligible for Executive Director review." : `Select an exact ${isPreliminary ? "Preliminary" : "Final"} Report version to review.`}</b><span>{report?.status === "DEPARTMENT_REVIEW" ? `${report.reportVersionId} is DEPARTMENT_REVIEW. Department Manager must forward the exact version, then General Manager must forward it before Executive Director review.` : report?.status === "GM_REVIEW" ? `${report.reportVersionId} is GM_REVIEW. General Manager must forward the exact version before Executive Director review.` : report ? `${report.reportVersionId} is ${report.status}; the current source stage must complete its next action before Executive Director review.` : "Select an exact server-owned version from the queue."}</span></section>}
    </div>
  </WorkspaceShell>;
}
...
export function ExecutiveReportPreviewPage() {
  ...
  useEffect(() => {
    if (!reportVersionId) {
      setError("The report preview route does not contain a report version identity.");
      return;
    }
    void Promise.all([
  ...
  return <WorkspaceShell roleLabel="Executive Director" routeLabel="Final Report Preview">
    <div className="executive-report-preview-page" data-report-version-id={report?.reportVersionId} data-testid="executive-report-preview-page">
      <header className="executive-report-preview-head workbench-page-header"><div><Link to="/executive-director/final-reports">← Return to Final Report review</Link><span>Selected report · {report?.reportId ?? "Loading"}</span><h1>Final Report Preview</h1><p>Server-owned immutable report version</p></div><div className="executive-preview-actions"><div aria-label="Preview zoom" className="executive-zoom-control"><span>Zoom</span>{[75, 90, 100, 110].map((value) => <button aria-pressed={zoom === value} className={zoom === value ? "is-active" : ""} key={value} onClick={() => setZoom(value)} type="button">{value}%</button>)}</div>{document?.downloadUrl ? <a href={document.downloadUrl} rel="noreferrer" target="_blank">Open generated document</a> : <span title="No generated document URL is available for this report version.">Generated document unavailable</span>}</div></header>
      <CommandError message={error} />
