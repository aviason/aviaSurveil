...
function AuditPlanSummary({ step, inspectionType, draft, values, option, estimate, autosaveState, autosaveError, onRetry }: { step: number; inspectionType: string; draft: PlanningProposalDraftView | null; values: NewAuditFormValues | null; option: CanonicalAuditScopeOption | null; estimate: PlanningWorkloadEstimate | null; autosaveState: AutosaveState; autosaveError: string | null; onRetry: () => void }) {
  ...
  if (inspectionType) facts.push(["Inspection type", catalogValueLabel(inspectionType)]);
  if (step >= 2 && values?.purpose.trim()) facts.push(["Purpose", values.purpose.trim()]);
  if (step >= 3 && values?.plannedDate) facts.push(["Planned date", readableDate(values.plannedDate)]);
  if (step >= 3 && values?.mode) facts.push(["Mode", values.mode]);
  if (step >= 3 && values?.mode === "On-site" && draft?.location?.label) facts.push(["Location", draft.location.label]);
  if (step >= 3 && values?.mode === "Remote" && values.meetingLink.trim()) facts.push(["Meeting link", values.meetingLink.trim()]);
  if (step >= 4 && values?.requiredInspectorCount) facts.push(["Required inspectors", values.requiredInspectorCount]);
  if (step >= 4 && values?.estimatedChecklistItemCount) facts.push(["Estimated checklist items", values.estimatedChecklistItemCount]);
  if (step >= 4 && values?.requestedBudget.trim()) facts.push(["Requested budget", `${values.requestedBudget} ${values.currency}`]);
  const body = <dl className="planning-intake-brief__facts"><div><dt>Inspected Organization</dt><dd>{organization}</dd></div>{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}{step >= 4 && estimate ? <div><dt>Workload basis</dt><dd>{estimate.basisLabel}</dd></div> : null}</dl>;
  const header = <header><div><h2>Plan summary</h2><p>Updates as you complete each step.</p></div><AutosaveIndicator state={draft ? autosaveState : "clean"} error={autosaveError} onRetry={onRetry} /></header>;
...
function SubmissionReadiness({ draft, values, autosaveState, autosaveError, onRetry }: { draft: PlanningProposalDraftView; values: NewAuditFormValues; autosaveState: AutosaveState; autosaveError: string | null; onRetry: () => void }) {
  const checks = [
    ["Scope complete", Boolean(draft.organizationName && draft.providerScopeLabel && draft.regulatedTargetLabel && values.inspectionType)],
    ["Purpose complete", Boolean(values.purpose.trim())],
    ["Schedule complete", Boolean(values.plannedDate && (values.mode === "Remote" || draft.location?.label))],
    ["Resources complete", Number(values.requiredInspectorCount) > 0 && Number(values.estimatedChecklistItemCount) > 0],
    ["Budget complete", values.requestedBudget.trim() !== ""],
  ] as const;
  const body = <><ul className="planning-intake-readiness__checks">{checks.map(([label, complete]) => <li className={complete ? "is-complete" : "is-incomplete"} key={label}><span aria-hidden="true">{complete ? "✓" : "!"}</span>{label}</li>)}</ul><div className="planning-intake-readiness__path"><strong>Approval path</strong><p>Finance Review → General Manager → Executive Director → General Manager Release</p></div></>;
  const header = <header><div><h2>Submission readiness</h2><p>Ready for Finance review.</p></div><AutosaveIndicator state={autosaveState} error={autosaveError} onRetry={onRetry} /></header>;
  return <aside aria-label="Submission readiness" className="planning-intake-brief planning-intake-readiness"><div className="planning-intake-brief__desktop">{header}{body}</div><details className="planning-intake-brief__mobile"><summary><span>Submission readiness</span><AutosaveIndicator state={autosaveState} error={autosaveError} onRetry={onRetry} /></summary>{body}</details></aside>;
}
...
function WorkloadPreview({ open, onClose, allRows, suggestedQuestions, busy, loadingMore, query, onQuery, view, onViewChange, total, suggestedTotal, allTotal, nextCursor, suggestedCount, safeMinimum, safeMaximum, onLoadMore, returnFocusRef }: WorkloadPreviewProps) {
  ...
  const showLoading = busy && (!isSuggested || suggestedQuestions.length === 0);
...
function NewAuditWizardPage() {
  ...
  const targetOptions = useMemo(() => [...new Map(scopeOptions.filter((option) => option.organizationId === (values?.organizationId ?? pendingOrganizationId) && option.providerScopeId === (values?.providerScopeId ?? pendingProviderScopeId)).map((option) => [option.regulatedTargetId, option])).values()], [pendingOrganizationId, pendingProviderScopeId, scopeOptions, values?.organizationId, values?.providerScopeId]);
  ...
  return <WorkspaceShell roleLabel="Department Manager" routeLabel={`New Audit · ${currentDefinition.label}`}>
    <div className="planning-intake-page planning-intake-page--new-audit" data-draft-id={draft?.id} data-testid="new-audit-wizard-page">
      <div className="planning-intake-workspace">
      <header className="planning-intake-header workbench-page-header"><div><h1>New Audit</h1><p>Create an audit plan for Finance review. Final checklist selection and inspector assignments happen after approval.</p></div></header>
      ...
      <div className="planning-intake-layout">
        <section aria-label="New Audit form" className="planning-intake-form">
          ...
          {step === 5 && values && draft ? <div className="planning-intake-review">
            ...
            <section className="planning-intake-review-section"><header><h3>Approval context</h3></header><p>Initiated by {draft.initiatedBy}. {noticeLabel(draft.noticePolicy)}.</p><p>Submit creates a Planning item for Finance Review. It does not create an executable Audit, final checklist, or Inspector assignment.</p></section>
          </div> : null}
        </section>
        {step === 5 && draft && values ? <SubmissionReadiness draft={draft} values={values} autosaveState={autosaveState} autosaveError={autosaveError} onRetry={() => void retryAutosave()} /> : <AuditPlanSummary step={step} inspectionType={values?.inspectionType ?? pendingInspectionType} draft={draft} values={values} option={selectedOption} estimate={estimate} autosaveState={autosaveState} autosaveError={autosaveError} onRetry={() => void retryAutosave()} />}
      </div>
