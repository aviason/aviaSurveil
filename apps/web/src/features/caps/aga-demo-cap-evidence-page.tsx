...
export function AGADemoCAPEvidencePage({
  ...
  const closeReason = lifecycleDisabledReason(capability, workspace.client, projection, managerAllowed, finding?.state === "PENDING_CLOSURE" ? (closureExplanation.trim() ? "" : "A bounded authorized-closure explanation is required.") : "Authorized closure is available only from PENDING_CLOSURE after accepted Evidence or the explicit no-Evidence branch.") ?? "";

  return (
    <LifecyclePageFrame
      description={auditeeAllowed ? "Submit immutable CAP and Evidence metadata with a public response comment. CAA verification and closure are published only as released status." : "Matching Service Provider submits immutable CAP/Evidence metadata; the CAA reviewer, verifier, and scoped Manager retain separate authorities."}
      error={workspace.error}
      eyebrow="Synthetic CAP and Evidence lifecycle"
      roleLabel={roleLabel}
      status={workspace.status}
      testId="aga-demo-cap-evidence-page"
      title={auditeeAllowed ? "CAP and Evidence" : "CAP, Evidence, and closure"}
    >
      <section aria-label="Finding selection" className="aga-lifecycle-facts">
        ...
        <article><span>CAP</span><strong>{cap?.state ?? "Not submitted"}</strong><small>{cap ? `revision ${cap.revision}` : auditeeAllowed ? "Latest public status" : "Append-only revisions"}</small></article>
      ...
      {auditeeAllowed ? <section aria-label="Public response comment" className="aga-lifecycle-panel">
        <h2>Public response comment</h2>
        <p>This comment is sent with the CAP or Evidence submission. Internal CAA notes are never projected to the Auditee.</p>
      ...
      {auditeeAllowed ? null : <section aria-label="CAA CAP review" className="aga-lifecycle-panel">
      ...
      {auditeeAllowed ? <section aria-label="Current public CAP and Evidence status" className="aga-lifecycle-panel">
        <h2>Current public CAP and Evidence status</h2>
        <p>CAP: {cap?.state ?? "Not submitted"}. Evidence: {evidence?.reviewState ?? "Not submitted"}.</p>
        <p>The current Finding remains open until the CAA publishes its separately authorized lifecycle outcome.</p>
      </section> : <>
      ...
      <section aria-label="Authorized closure" className="aga-lifecycle-panel">
        ...
        {managerAllowed ? <label>Authorized closure explanation<textarea aria-label="Authorized closure explanation" maxLength={1000} value={closureExplanation} onChange={(event) => setClosureExplanation(event.target.value)} /></label> : null}
        <LifecycleAction actionId="authorized-close" disabled={Boolean(closeReason) || workspace.pending} label="Authorize Finding closure" onClick={() => void workspace.runCommand("AUTHORIZED_CLOSE", { findingId: selectedFindingId, reasonCode: "MANAGER_AUTHORIZED_CLOSURE", reasonExplanation: closureExplanation.trim() }).catch(() => undefined)} reason={closeReason || "The command is pending."} />
      </section>
