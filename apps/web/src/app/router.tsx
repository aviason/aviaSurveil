...
import { CANONICAL_AUDIT_PREPARATION_PATH, CANONICAL_INSPECTOR_AUDIT_PATH, CANONICAL_INSPECTOR_CHECKLIST_PATH, REACT_ROUTE_CONTRACT_BY_ID, REACT_ROUTE_CONTRACTS, type BuildProfileAvailability, type ReactSurfaceId, type RouteContract } from "./route-contracts";
...
function RoleSelectRoute() {
  const { identityMode } = useApplicationRuntime();
  ...
  return <RoleSelectPage mode={identityMode ?? "oidc-session"} onRoleRequest={(role) => navigate(createRoleEntryPath(role))} />;
}
...
function ContractRoute({ contract }: { contract: RouteContract }) {
  ...
  return <RoleGuard additionalRoles={contract.additionalRoles} requiredRole={contract.requiredRole}>{!isAvailable ? <BlockedProfileRoute contract={contract} /> : entry.status === "implemented" ? <entry.component /> : <PendingImplementationRoute contract={contract} />}</RoleGuard>;
}
