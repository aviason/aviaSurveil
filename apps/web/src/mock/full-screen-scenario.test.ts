...
describe("full-screen deterministic demo scenario", () => {
  it("keeps an exhaustive source-faithful action map with non-inert high-risk effects", () => {
    ...
    expect(SCREEN_VISIBLE_ACTIONS["new-audit-wizard-5"].map((action) => action.id)).toEqual(["wizard-back", "wizard-submit"]);
  ...
  it("loads all 86 active role-safe projections from one seed and exposes every declared visible command", async () => {
