-- Frozen already-ledgered version-21 fixture captured before Task 6 decision
-- root pinning, current-effective actor validation, and mapping ordinals.
-- Tests execute this resource directly; it must not be derived from the
-- current migration at runtime.
...
-- TASK 6 FIX ROUND 4 FROZEN EDITED SUCCESSOR DATA
--
-- The harness applies this checked-in data section only after the preceding
-- schema and the frozen root history have been installed. It models an
-- already-reviewed successor whose inherited mapping identities are
-- non-lexical and whose new mapping is recoverable only from the ordered
-- question snapshot.
...
VALUES (
    'QV-CAND-TASK6-FIX4-EDITED',
    'Q-TASK6-FIX4-EDITED',
    2,
    'Can the inspector reconcile the synthetic ramp-safety evidence with the controlled synthetic requirement?',
    'MAP-Z-FROZEN-FIRST,MAP-A-FROZEN-SECOND,MAP-N-FROZEN-APPENDED',
    'Synthetic inspection observation,Synthetic controlled record',
    'USR-TASK6-FIX2-FROZEN-ADMIN'
);
...
SELECT
    ...
    owner_role,
    'USR-TASK6-FIX2-FROZEN-ADMIN',
    'Frozen checked-in edited successor.',
    ARRAY['QV-CAND-TASK6-FIX4-EDITED'],
    revision + 1,
    generation_run_id,
    'sha256:97a386e78b9555dd3eec5be0b7e499d5ab6236d7285c5a5a4faad80137ba762c',
    candidate_schema_version,
    candidate_root_id,
    id
...
WHERE candidate_draft_version_id = 'CAND-TASK6-FIX2-FROZEN'
  AND mapping_id IN ('MAP-Z-FROZEN-FIRST', 'MAP-A-FROZEN-SECOND');
...
SELECT
    ...
    jsonb_set(snapshot, '{mappingId}', '"MAP-N-FROZEN-APPENDED"'::jsonb)
FROM regulatory_generated_mapping_snapshots
WHERE candidate_draft_version_id = 'CAND-TASK6-FIX2-FROZEN'
  AND mapping_id = 'MAP-Z-FROZEN-FIRST';
...
VALUES (
    ...
    'Q-TASK6-FIX4-EDITED',
    '{
      "questionId": "Q-TASK6-FIX4-EDITED",
      "mappingIds": [
        "MAP-Z-FROZEN-FIRST",
        "MAP-A-FROZEN-SECOND",
        "MAP-N-FROZEN-APPENDED"
...
VALUES (
    ...
    'DEPARTMENT_REVIEW',
    'TECHNICALLY_APPROVED',
    'Frozen checked-in edited successor approval.',
    'TASK6-FIX4-EDITED-APPROVE',
    'TASK6-FIX4-EDITED-APPROVE',
    'TASK6-FIX4-EDITED-APPROVE',
    '{}'::jsonb
);
...
INSERT INTO department_review_decisions (
    ...
    decision,
    actor_subject_id,
    actor_department_membership_id,
    actor_department_id,
    actor_organizational_unit_id,
    reason,
    decided_at,
    operation_id,
    idempotency_key,
...
-- END TASK 6 FIX ROUND 4 FROZEN EDITED SUCCESSOR DATA
