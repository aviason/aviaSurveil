...
func TestPackageReaderRejectsRelativePathBeforeOpeningArchive(t *testing.T) {
	reader := canonicalaga.NewPackageReader()
...
func TestAcceptedPackage(t *testing.T) {
	...
	accepted, err := canonicalaga.NewPackageReader().ReadAndValidate(
		context.Background(), packagePath, canonicalaga.ExactAcceptedPackage(),
	)
	if err != nil {
...
func syntheticExpected(t *testing.T) canonicalaga.ExpectedPackage {
	t.Helper()
	return canonicalaga.ExpectedPackage{
		PackageVersion: "SYNTHETIC_AGA_DEMO_V1",
		PackageStatus:  "PENDING_ADMIN_AND_SOURCE_OWNER_REVIEW",
		ExpectedCounts: canonicalaga.ExpectedCounts{
...
func writeSyntheticPackage(t *testing.T, mutation string) (string, canonicalaga.ExpectedPackage) {
	t.Helper()
