...
func (api *CanonicalAPI) Handler() http.Handler {
	...
	router.Get("/v1/planning/purpose-presets", api.listPlanningPurposePresets)
	router.Get("/v1/planning/locations", api.listPlanningLocations)
	router.Post("/v1/planning/location-resolutions", api.resolvePlanningLocation)
	router.Post("/v1/planning/workload-estimates", api.getPlanningWorkloadEstimate)
	router.Post("/v1/planning/proposal-drafts", api.createPlanningProposalDraft)
	router.Get("/v1/planning/proposal-drafts/{draftId}", api.getPlanningProposalDraft)
	router.Put("/v1/planning/proposal-drafts/{draftId}", api.savePlanningProposalDraft)
	router.Post("/v1/planning/proposal-drafts/{draftId}/submissions", api.submitPlanningProposal)
	router.Get("/v1/planning/items/{planningItemId}/audit-package-setup", api.getPlanningAuditPackageSetup)
	router.Post("/v1/planning/items/{planningItemId}/audit-package-setup", api.ensurePlanningAuditPackageSetup)
	router.Post("/v1/planning/items/{planningItemId}/audit-package-finalizations", api.finalizePlanningAuditPackage)
