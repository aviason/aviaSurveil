...
// required storage contract for the canonical first-party runtime.
...
func (storage *PostgresStorage) GetClientByClientID(ctx context.Context, id string) (op.Client, error) {
	...
	return memoryClient{id: id, redirectURI: redirects[0], postLogoutRedirect: logouts[0], loginURL: providerEndpoint(storage.candidate.Issuer, "login")}, nil
}
...
func scopesAllowed(scopes []string) bool {
	for _, scope := range scopes {
		if !containsScope([]string{oidc.ScopeOpenID, oidc.ScopeProfile, oidc.ScopeEmail}, scope) {
			return false
		}
	}
	return true
...
func (storage *PostgresStorage) SetUserinfoFromRequest(ctx context.Context, info *oidc.UserInfo, request op.IDTokenRequest, scopes []string) error {
	if err := storage.setUserinfo(ctx, info, request.GetSubject(), scopes); err != nil {
		return err
	}
	claims, err := storage.privateClaims(ctx, request.GetSubject())
	if err != nil {
		return err
	}
	for key, value := range claims {
		info.AppendClaims(key, value)
	}
	return nil
}
...
func (storage *PostgresStorage) setUserinfo(ctx context.Context, info *oidc.UserInfo, subject string, scopes []string) error {
	...
	if containsScope(scopes, oidc.ScopeProfile) {
		...
		if err := storage.pool.QueryRow(ctx, `SELECT display_name FROM auth_identity.provider_profiles WHERE subject_id = $1`, subject).Scan(&displayName); err != nil {
...
func (storage *PostgresStorage) GetPrivateClaimsFromScopes(ctx context.Context, subject, _ string, _ []string) (map[string]any, error) {
	return storage.privateClaims(ctx, subject)
}

func (storage *PostgresStorage) privateClaims(ctx context.Context, subject string) (map[string]any, error) {
	var organizationID, role, membershipID, state string
	var membershipRevision int64
	var authRevision uint64
	var emailVerified bool
	err := storage.pool.QueryRow(ctx, `
		SELECT aa.organization_id, aa.role, aa.membership_id, aa.state,
		       aa.membership_revision, a.auth_revision, a.email_verified
		FROM auth_identity.accounts a
		JOIN auth_identity.application_authorities aa ON aa.subject_id = a.subject_id
		WHERE a.subject_id = $1 AND a.state = 'active' AND a.email_verified = true
		  AND aa.state = 'ACTIVE'`, subject).Scan(
		&organizationID, &role, &membershipID, &state,
		&membershipRevision, &authRevision, &emailVerified,
	)
	if errors.Is(err, pgx.ErrNoRows) || state != "ACTIVE" || !emailVerified {
		return nil, oidc.ErrUnauthorizedClient()
	}
	if strings.TrimSpace(organizationID) == "" || strings.TrimSpace(role) == "" || strings.TrimSpace(membershipID) == "" || membershipRevision < 1 || authRevision < 1 {
		return nil, oidc.ErrUnauthorizedClient()
	}
...
func (storage *PostgresStorage) SetAuthenticatedAMR(ctx context.Context, requestID string, amr []string) error {
...
// RevokeAllSessions invalidates every durable first-party credential for a
// subject. It is the callback used by password, MFA, authority, and lifecycle
// security mutations.
func (storage *PostgresStorage) RevokeAllSessions(ctx context.Context, subjectID string) error {
	now := storage.clock().UTC()
	tx, err := storage.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin provider credential revocation: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	if _, err := tx.Exec(ctx, `UPDATE auth_identity.oidc_access_tokens SET state = 'revoked', revoked_at = $2 WHERE subject_id = $1 AND state = 'active'`, subjectID, now); err != nil {
		return fmt.Errorf("revoke provider access tokens: %w", err)
	}
	if _, err := tx.Exec(ctx, `UPDATE auth_identity.oidc_refresh_tokens SET state = 'revoked', revoked_at = $2 WHERE subject_id = $1 AND state = 'active'`, subjectID, now); err != nil {
		return fmt.Errorf("revoke provider refresh tokens: %w", err)
	}
	if _, err := tx.Exec(ctx, `UPDATE auth_identity.provider_sessions SET state = 'revoked', revoked_at = $2 WHERE subject_id = $1 AND state = 'active'`, subjectID, now); err != nil {
		return fmt.Errorf("revoke provider sessions: %w", err)
	}
