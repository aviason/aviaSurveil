...
fail() {
  printf 'canonical-preprod-cloudflare-lifecycle: %s\n' "$*" >&2
  exit 1
}
...
output_root="$(mktemp -d /private/tmp/aviasurveil360-canonical-quick-tunnel-lifecycle.XXXXXX)"
cleanup() {
  case "$output_root" in
    /private/tmp/aviasurveil360-canonical-quick-tunnel-lifecycle.*) rm -rf -- "$output_root" ;;
    *) printf 'canonical-preprod-cloudflare-lifecycle: refusing unsafe output cleanup\n' >&2 ;;
  esac
}
trap cleanup EXIT
...
(
  ...
  AVIA_PLAYWRIGHT_OUTPUT_DIR="$output_root" \
    ./node_modules/.bin/playwright test \
      tests/e2e/canonical-quick-tunnel-lifecycle.spec.ts \
      --project=canonical-quick-tunnel
)

printf 'canonical Quick Tunnel 1,310-question lifecycle verified locally at %s; external preprod not run.\n' "$public_origin"
