...
node --input-type=module - "$runtime_file" "$fixture_file" "$password_file" <<'NODE'
import { readFileSync } from "node:fs";

const [runtimePath, fixturePath, passwordPath] = process.argv.slice(2);
...
if (
  ...
  throw new Error("verified Quick Tunnel login metadata is invalid");
}
