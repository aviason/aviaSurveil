...
| `deploy/local/` | Local verification services | Test/local profiles only; no production deployment authority. The collector-backed full profile keeps browser OTLP enabled and routes `/otel/*` to the private collector; canonical local-preprod and Quick Tunnel builds provision no collector and explicitly compile browser telemetry off instead of silently returning 405 or dropping signals. |
