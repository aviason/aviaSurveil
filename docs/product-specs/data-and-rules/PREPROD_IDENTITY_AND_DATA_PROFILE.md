...
## Historical AGA candidate donor contract (removed)

The former `aga-candidate-demo@1.1.0` overlay is historical evidence only. Its
schema provisioners, tagged runtime, API/UI, commands, scripts, and tests were
physically removed after canonical Task 9 qualification and the user's
explicit `delete` decision. It is not a current profile, route, capability,
data store, or execution instruction and created no real candidate, identity,
assignment, source attestation, decision, publication, delivery, Finding,
Audit, release, or production record. Historical package receipts remain in
their owning paused plans; current exercise behavior is governed solely by the
canonical profile boundary above.
