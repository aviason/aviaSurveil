...
Scope includes Evidence and inspection-attachment scan requests, ClamAV
availability, immutable object metadata, and worker delivery state.
...
- ClamAV is unhealthy or signature metadata is stale.
...
docker compose --project-name "$AVIA_LOCAL_PROJECT" --file deploy/local/compose.yaml --profile full ps --all worker clamav minio
...
Healthy processing drains ready scan messages. Clean, infected, and error
results remain distinct, include scan provenance, and never convert a pending
state directly into reviewed Evidence.
...
scanner/signature version, timestamps, correlation ID, dependency health, and
final recovery status. Do not attach object bytes.
